# load cyphertext
cipher1 = ""
cipher2 = "0x234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f"
cipher3 = "0x32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb"
cipher4 = ""
cipher5 = "0x3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070"
cipher6 = ""
cipher7 = ""
cipher8 = ""
cipher9 = ""
cipher10 = "0x466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83"
target_cipher = "0x32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904"
cipher_array= [cipher1, cipher2, cipher3, cipher4, cipher5, cipher6, cipher7, cipher8, cipher9, cipher10, target_cipher]
# hàm trả về mảng các giá trị hex đơn từ 1 chuỗi hex
def getHex_val(str):
    hex_val_arr = []
    for i in range(2, len(str), 2):
        hex_val = str[i] + str[i+1]
        hex_val_arr.append(hex_val)
    return hex_val_arr
# hàm xor các ký tự (a-z A-Z) với dấu cách và trả về danh sách kết quả đã xor
def spaceXORCharacter():
    space_XOR_with_character = []
    for character in range(97,122):
        space_XOR_with_character.append(hex(int(hex(character), 16) ^ 0x20))
    for character in range(65,91):
        space_XOR_with_character.append(hex(int(hex(character), 16) ^ 0x20))
    space_XOR_with_character.append("0x00")
    return space_XOR_with_character
#hàm này trả về một khóa (key) dưới dạng chuỗi hexadeciaml, 
#được tính toán bằng cách thực hiện phép XOR giữa giá trị hexadeciaml đại diện cho một ký tự trong chuỗi mã hóa và giá trị hexadeciaml của ký tự space.
def getKeyFromXORedValue(current_cipher_array, hex_index):
    return hex(int("0x" + current_cipher_array[hex_index], 16) ^ 0x20) 

def decryptCipherWithKey(cipher, key_array):
    cipher_array = getHex_val(cipher)
    message = []
    for i in range(min(len(cipher_array), len(key_array))): 
        if key_array[i] is None:
            message.append("_")
        else:
            hex_val = hex(int("0x" + cipher_array[i], 16) ^ int(key_array[i], 16))
            message.append(chr(int(hex_val, 16)))
    return " ".join(message)

def main():
    space_XOR_with_character = spaceXORCharacter() #tạo 1 danh sách các giá trị hex sau khi XOR space với các ký tự a-z A-Z
    key_store = [None for i in range(200)] # khởi tạo key_store lưu trữ key

    #duyệt qua các cyphertext xor chúng với nhau
    for cipher_index, current_cipher in enumerate(cipher_array):
        hex_arr1 = getHex_val(current_cipher)
        supicious_index = {} #tạo 1 từ điển lưu trữ các chỉ số nghi ngờ (là khoảng trắng hoặc ký tự chữ cái bình thường)
        for compared_cipher_index, compared_cipher in enumerate(cipher_array):
            if compared_cipher != current_cipher:
                hex_arr2 = getHex_val(compared_cipher)
                # xor 2 ciphers together
                for i in range(min(len(hex_arr1), len(hex_arr2))): #lấy min 2 mảng để xor
                    hex_val = hex(int("0x" + hex_arr1[i], 16) ^ int("0x" + hex_arr2[i], 16)) #lưu giá trị sau khi xor dưới dạng hex
                    if hex_val in space_XOR_with_character: #check xem kết quả hex vừa rồi có trong danh sách space_XOR_with_character không
                        if supicious_index.get(i) is None:
                            supicious_index[i] = 1
                        else:
                            supicious_index[i] += 1
                        # nếu chỉ số xuất hiện nhiều lần (>7 - đáng ngờ) từ đó dự đoán key 
                        if supicious_index[i] > 7:
                            key_store[i] = getKeyFromXORedValue(hex_arr1, i)
    # print(key)
    print("Secret message is guessed below, you need to guess the missing letters to finnish the message: \n" )
    print(decryptCipherWithKey(target_cipher, key_store))
    print("\n\nNOTE: the '_' character can be the placeholder for a alphanumeric character or a space!")



#run main
main()
